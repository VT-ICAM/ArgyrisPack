...
[C2 B2 b2 Th2] = ArgyrisMapsMex(someTriangle);

argyris = evalArgy(C, qV);
argyris2 = ArgyrisFunctionsMex(C2, qV.values);

[dx dy] = evalGradArgy(C, B, qV);
[dx2 dy2] = ArgyrisGradientsMex(C2, B2, qV.derivativeX, qV.derivativeY);

[dxx dxy dyy] = evalHessArgy(C, Th, qV);
[dxx2 dxy2 dyy2] = ArgyrisHessiansMex(C2, Th2, qV.derivativeXX, qV.derivativeXY, ...
                                   qV.derivativeYY);

disp('max difference in function values:')
max(max(abs(argyris - argyris2)))
